...
function sendMessageFromButton(buttonText) {
  console.log('sendMessageFromButton', botBusy);
  if (botBusy) return;
  botBusy = true;
...
// console.log('hiddenButtonElements =', hiddenButtonElements);
hiddenButtonElements.forEach((button) => {
  button.addEventListener('click', function (event) {
    // event.stopPropagation();
    // const buttonText = this.textContent;
    // sendQuestion(buttonText);
    // hiddenButtons.style.display = 'none';
  });
});
...
document
  ...
  .addEventListener('click', function (event) {
    ...
    console.log('hello-toogle button');
...
document.addEventListener('click', function (event) {
  ...
  if (
    ...
    event.target !== toggleTrigger &&
    event.target !== toggleLabel
  ) {
    toggleSidebar();
  }
});
